...
Wagr is a peer-to-peer prediction protocol that turns live market events into instant head-to-head duels.
...
Zero-click settlement requires native execution-layer reactivity: Somnia's precompile is currently the cleanest implementation of this at the protocol level. The WagrEscrow architecture is chain-agnostic; Ethereum and Solana support are on the roadmap as equivalent reactivity primitives mature on those networks.
...
2. **Embed or Share Widget Link (`/widget/[creator]`)**:
   - The creator embeds their custom widget URL (e.g. `/widget/kaicenat`) into OBS browser sources, Twitch panels, or pinned live chat messages.
...
*Submitted to BUILD//ANYTHING 2026 - sponsored by Anna (anna.partners)*
